...
myDomain = RZDomain_GPU(Nr,Nz,dR,dZ,boundaryCondition)
...
function main_loop()
    while time < maxTime
        ...
        println("Particle count check: ", @btime CUDA.sum($myParticle.alive))
        if CUDA.sum(myParticle.alive) >= 0.9*length(myParticle.alive)
            println("Enlarge particle array: ", @btime enlargeParticleArraySize!($myParticle))
        end

        println("Particle count cylindrical: ", @btime particleCountCylindrical($myDomain, $myParticle))
        
        println("Poisson cylindrical: ", @btime PoissonCylindricalCUDSSNEW($myDomain, $v_0))

        println("Xenon neutral collisional ionization: ", @btime XenonNeutralCollisionalIonization_optimized($myParticle, $myDomain, $timeStep, $EList, $TList, $WList, $σList, $σLength, $stepT))
        
        println("Neutral collisions: ", @btime neutralCollisions($myParticle, $timeStep, 2.1802e-25, 216.0e-12))
        
        println("Set magnetic field: ", @btime setMagneticField($myParticle, $myMagneticField))

        println("Electric field cylindrical: ", @btime electricFieldCylindrical($myParticle, $myDomain))
        
        println("Boris pusher: ", @btime BorisPusher($myParticle, $timeStep))
        
        println("Absorb BE reflect plasma BC: ", @btime absorbBE_reflectPlasma_BC_countThrust($myParticle, $myDomain))
        
        println("Total energy: ", @btime totalEnergy($myParticle, $myDomain, $myData))
        ...
        if Ne_dot_max > 0
            ...
            if BeamSuperSize > 0
                println("Create beam particle: ", @btime createBeamParticle_GPU($BeamSuperParticle, $me, 0.0, -$qe, $BeamSuperSize, 0.0, $CuArray([0,rb]), $CuArray([dZ/Nz/1000,dZ/Nz/999]), $myParticle, $Ue))
            end
        end
      
        if neutralRate > 0
            println("Neutral source: ", @btime neutralSource($neutralRate, $timeStep, $myParticle))
        end
        
        println("Accumulate output data: ", @btime accumualteOutputData($myDomain, $myData))
        if counter >= maxCounter
            println("Write to file: ", @btime writeToFile($myData, $counter, $output_folder, $time, $timeStep))
            counter = 0
        else
            counter += 1
        end
        
        println("Find alive particles: ", @btime CUDA.findall(x -> x==1, $myParticle.alive))
        aliveidx = CUDA.findall(x -> x==1, myParticle.alive)
        println("Find electron subindex: ", @btime CUDA.findall(x -> x<0, $myParticle.qArray[$aliveidx]))
        electronsubidx = CUDA.findall(x -> x<0, myParticle.qArray[aliveidx])
        electronidx = aliveidx[electronsubidx]
        println("Sum electron super particles: ", @btime CUDA.sum($myParticle.superParticleSizeArray[$electronidx]))
        ...
        time += timeStep
        println("time= ", time, ", Particle count: ", CUDA.sum(myParticle.alive))
    end
end
