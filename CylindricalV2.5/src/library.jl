...
function construct_BNEW(rho, eps0, phi, phiz_first)
    ...
    fill_B!(B, rho, eps0, phiz_first, Nr, Nz))
    
    return B
end

# Kernel function to fill B
function fill_B!(B, rho, eps0, phiz_first, Nr, Nz)
    i = threadIdx().x + (blockIdx().x - 1) * blockDim().x
    j = threadIdx().y + (blockIdx().y - 1) * blockDim().y
...
function PoissonCylindrical(myDomain, phiz_first::Float64)
    ...
    qe =  1.6021766208e-19;
    #construct B
    B = construct_B(myDomain.charge_bin./myDomain.nodeVolume, eps0, myDomain.phi, phiz_first)
...
function PoissonCylindricalCUDSSNEW(myDomain, phiz_first::Float64)
    ...
    B = construct_BNEW(myDomain.charge_bin./myDomain.nodeVolume , eps0, myDomain.phi, phiz_first)
        ...
        if alive[idx] == 1
            ...
            if z > z_MAX[1] || z < 0.0
                ...
                #periodic
                    ...
                    @inbounds PosArray[3,idx] -= z_MAX[1]
                    @inbounds PosArray_old[3,idx] = PosArray[3,idx]
                else
            ...
            while sqrt( PosArray[1,idx]^2 + PosArray[2,idx]^2) >= r_MAX[1]
                PosArray[1,idx] *= 0.99
                PosArray[2,idx] *= 0.99
                @inbounds PosArray_old[1,idx] = PosArray[1,idx]
                @inbounds PosArray_old[2,idx] = PosArray[2,idx]
...
function enlargeParticleArraySize!(myParticle)
    println("Approaching particle number limit, enlarge limit by 10%")
    # 1. Get old array size
    Nmax_old = length(myParticle.alive)
    
    # 2. Initialize new object with enlarged size
    Nmax_new = Int(ceil(Nmax_old * 1.1))
    myParticle_new = Particle_GPU(Nmax_new)
    
    # 3. Copy old data to new object
    for field in fieldnames(Particle_GPU)
        old_field = getfield(myParticle, field)
        new_field = getfield(myParticle_new, field)
        if ndims(old_field) == 1
            new_field[1:Nmax_old] .= old_field
        else
            new_field[:, 1:Nmax_old] .= old_field
        end
        setproperty!(myParticle, field, new_field)
    end
    
    # 4. Delete the temporary new object
    myParticle_new = nothing
    global Nmax = Nmax_new
    println("Particle has enlarged")
end

function neutralSource(neutralRate,timeStep,myParticle)
    aliveIdx = CUDA.findall(x->x==1, myParticle.alive)
    neutralSubIdx = CUDA.findall(x->x==0, myParticle.qArray[aliveIdx])
    neutralIdx = aliveIdx[neutralSubIdx]
    #Divide total number increase to each neutral particle
    myParticle.superParticleSizeArray[neutralIdx] .+= neutralRate*timeStep/length(neutralIdx);
end
