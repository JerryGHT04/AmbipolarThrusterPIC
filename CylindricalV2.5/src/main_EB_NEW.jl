...
saveFrequency = parse(Float64, parameters[26])
#--------------------------------------------------------------------------#
# constant
me =  9.10938356E-31;
qe =  1.6021766208e-19;
...
maxCounter_save = Int(floor(saveFrequency / timeStep))
...
while time < maxTime
    ...
    Numionized = XenonNeutralCollisionalIonization_optimized(myParticle, myDomain, timeStep, EList, TList, WList, σList)
    ...
    if counter_save >= maxCounter_save
        saveData(save_path, time, myParticle, myDomain, myMagneticField)
        print("File saved!")
        counter_save = 0
    else
        counter_save += 1
